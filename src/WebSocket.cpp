...
void WebSocket::_readFrame() {
  ...
  default: {
    __debugOutput(("Unrecognized frame opcode: %u\n"), header.opcode);
    close(PROTOCOL_ERROR, true);
    break;
  }
  }
...
bool WebSocket::_readHeader(header_t &header) {
  ...
  if (header.rsv1 || header.rsv2 || header.rsv3) {
    __debugOutput(("Reserved bits should be empty!\n"));
    __debugOutput(("RSV1 = %d, RSV2 = %d, RSV3 = %d\n"), header.rsv1,
  ...
  if (isControlFrame(header.opcode)) {
    if (!header.fin) {
      __debugOutput(("Control frames must not be fragmented!\n"));
    ...
    if (header.length > 125) {
      __debugOutput(
        ("Control frames max length = 125, here = %lu\n"), header.length);
  ...
  if (header.length == 126) {
    ...
    __debugOutput(("Unsupported frame size!\n"));
  ...
  if (header.length > kBufferMaxSize) {
    __debugOutput(("Unsupported frame size = %lu\n"), header.length);
...
#ifdef _DUMP_HEADER
  printf(("RX FRAME : OPCODE=%u, FIN=%s, RSV=%d, PAYLOAD-LEN=%u, MASK="),
...
void WebSocket::_handleCloseFrame(const header_t &header, const char *payload) {
  ...
  __debugOutput(("Received close frame: code = %u, reason = %s\n"), code,
